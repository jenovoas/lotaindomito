...
| Estado | Decisiones |
|---|---|
| **Vigentes** (encuadran el proyecto hoy) | [D-006](#d-006--servidor-fase-1--python-fastapi), [D-007](#d-007--interfaz--vue-3--typescript), [D-011](#d-011--camino-c-confirmado--motor-propio-s60--pipeline-gpu-activo-2026-08-09), [D-012](#d-012--arquitectura-de-integracin-sentinel--gpu-confirmada-2026-08-09), [D-014](#d-014--encuadre-vigente-concepto-real-del-proyecto-2026-08-10-corregida-el-mismo-da), [D-016](#d-016--reemplazo-de-carboncillo-por-sistema-multi-moneda-de-minerales-2026-08-10-aprobada-el-2026-08-12), [D-017](#d-017--subastas-digitales-de-cosas-reales-con-pago-en-minerales-2026-08-10-aprobada-el-2026-08-12), [D-018](#d-018--wire-format-s60-componentes-explcitos-en-api-rest-y-websocket-2026-08-12), [D-019](#d-019--experiencia-de-juego-mmo-ra-urbano-pokmon-go--wow-con-patrullas-sincronizadas-2026-08-13), [D-020](#d-020--mochila-minera-y-bitcora-de-despachos-meso-loop-2026-08-13), [D-021](#d-021--sistema-de-diseo-hud-tctico-patrimonial--arquitectura-de-juego-explcita-en-3-capas-2026-08-14) |
...
- **Componentes:**
  1. **Categorías de Ítems:** Minerales & Combustibles (*Carbón Grasa*, *Pirita*), Fichas de Pulpería (*Ficha Cousiño 1895*), y Reliquias & Documentos Históricos (*Órdenes de Despacho Ferroviario*, *Cartas Lacradas de Isidora*, *Lámpara Davy*).
  2. **Sistema de Rareza y Lore:** Cada ítem cuenta con memoria histórica narrada en primera persona para reforzar la identidad patrimonial de Lota.
  3. **Bitácora de Misiones Activas:** Seguimiento de progreso, objetivo en metros/zona y entrega de recompensas vinculada a la billetera y geofencing.
- **Implementación:** `piloto-a/src/stores/inventory.ts` y componente `piloto-a/src/components/MochilaMinera.vue`.
...
- **Estado actual (2026-08-12):** APROBADA por INTERLOCUTOR. Sistema multi-moneda cobre/oro/estaño vigente, reemplaza a Carboncillo como mecánica oficial. Diseño completo en [`_analisis/23_sistema_monedas_minerales.md`](../_analisis/23_sistema_monedas_minerales.md). GDD actualizado con §4 sistema multi-moneda. Implementación del wallet pendiente de materializar en Piloto A y backend.
...
### D-021 · Sistema de diseño HUD táctico-patrimonial + Arquitectura de Juego explícita en 3 capas (2026-08-14)

- **Decisión:** introducir dos cambios acoplados en el Piloto A:
  1. **Sistema de diseño HUD "Táctico-Patrimonial":** tokens visuales centralizados en `piloto-a/src/assets/design-tokens.css` (paleta turbo-industrial, tipografía Space Grotesk + JetBrains Mono, biseles carbón, efectos de cristal). Componentes nuevos: `NpcAvatar.vue` (retratos SVG 2.5D de Isidora, Ciego, Chinchorrera, Palanquero), `BrumaCostera.vue` (niebla CSS), `EncuentroSheet.vue` (ficha de colección), `EncuentroPulso.vue` (anillo de zona), `BannerIntercept.vue` (banner con háptica). Identidad visual coherente entre landing e in-game.
  2. **Arquitectura de Juego explícita en 3 capas (Simulación → Estado → Presentación):** composable `useGameLoop` (rAF con pausa por `document.hidden`), `RingBuffer` con `sampleAt` para interpolación LERP de posiciones de GPS y NPC en `piloto-a/src/utils/interpolationBuffer.ts` (con tests Vitest), store `lattice` con reconnect exponencial+jitter y `connectionStatus`, `useGraphicsProfile` con detección `full` / `lite` / `css-only` aplicada automáticamente al cargar (memoria, dpr, WebGL). La capa de Presentación queda prohibida de llamar al backend sin pasar por la capa de Estado.
- **Razón:** el Piloto A funcionaba pero se sentía como una página web con mapa (HUD plana, marcadores tipo "pin de Google", sin coherencia con la propuesta de patrimonio) y la arquitectura mezclaba patrón web clásico con realidades de juego (eventos de lattice, NPCs móviles, GPS intermitente). El cambio explicita el modelo mental de juego para futuras sesiones, suaviza los saltos de GPS/NPC con interpolación, y eleva el nivel visual al de la propuesta. La degradación progresiva por perfil evita regresión en gama baja.
- **Contraparte analizada:** (a) continuar con la estética actual sin rediseñar (rechazado — la propuesta conceptual pide calidad de patrimonio); (b) introducir un game engine en el cliente (Phaser, PixiJS, Three.js on top of MapLibre — rechazado por peso y duplicación de render); (c) no separar capas y dejar watchers Vue directos (rechazado — stutters en eventos de lattice > 1 Hz).
- **Reversible:** sí, cada fase es un commit independiente. Los tokens pueden revertirse quitando el import en `main.ts`; el render loop puede volver a watchers eliminando `useGameLoop` y los buffers.
- **Estado actual (2026-08-14):** APROBADA e IMPLEMENTADA. Componentes, composables, store y landing actualizados. Diagrama y contrato WebSocket documentados en [`_analisis/26_arquitectura_game_loop_3_capas.md`](../_analisis/26_arquitectura_game_loop_3_capas.md). CHANGELOG con hitos verificables (commits, métricas de FPS, screenshots).

- **Estado actual (2026-08-12):** APROBADA por INTERLOCUTOR. Sistema de subastas digitales con pago en minerales vigente (sin pago en CLP). Diseño completo en [`_analisis/24_subastas_reales.md`](../_analisis/24_subastas_reales.md). GDD actualizado con §11 subastas digitales. Implementación del servicio de subastas pendiente en backend.
